use tokio_stream::{self as stream, pending, Stream, StreamExt};
...
async fn multiple_entries() {
    ...
    v.sort_unstable();
    assert_eq!(v[0].0, "bar");
    assert_eq!(v[0].1, Some(4));
    assert_eq!(v[1].0, "foo");
    assert_eq!(v[1].1, Some(3));
...
fn one_ready_many_none() {
    ...
    for _ in 0..100 {
        ...
        let mut results = vec![];
        while let core::task::Poll::Ready(Some((k, v))) = map.poll_next() {
            results.push((k, v));
        }
...
proptest::proptest! {
    ...
    fn fuzz_pending_complete_mix(kinds: Vec<bool>) {
        ...
        for _ in 0..10 {
            ...
            if expect == 0 {
                while let Poll::Ready(Some(v)) = map.poll_next() {
                    assert_eq!(v.1, None);
                }
                assert_ready_none!(map.poll_next());
            } else {
